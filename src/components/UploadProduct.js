...
const UploadProduct = ({
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const uploadApi = await fetch(summaryApi.uploadProduct.url, {
        ...
        const dataResponse = await uploadApi.json();
        if (!dataResponse.err) {
    ...
    return (
        <div className="fixed top-0 left-0  h-full w-full flex justify-center items-center bg-slate-100 bg-opacity-80">
            <div className="bg-white p-4 rounded w-full max-w-2xl h-full max-h-[80%] overflow-hidden">
                ...
                <form className="grid gap-1 productForm p-4 pb-6 h-full overflow-y-scroll" onSubmit={handleSubmit}>
                    ...
                    <button className='bg-red-500 my-2 py-2 text-white font-medium mb-8 hover:bg-red-700'>Upload Product</button>
                </form>
            </div>
